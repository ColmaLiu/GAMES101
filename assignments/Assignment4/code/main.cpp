...
void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) 
{
    ...
    for (float t = 0.0; t <= 1.0; t += 0.001)
    {
        ...
        vec.push_back(cv::Point2f(floor(point.x), floor(point.y)));
        ...
        for (int i = 0; i < 4; i++)
        {
            float d = max_d - std::sqrt(std::pow(vec[i].x + 0.5f - point.x, 2) + std::pow(vec[i].y + 0.5f - point.y, 2));
            ds.push_back(d);
            sum_d += d;
        }
        for (int i = 0; i < 4; i++)
        {
            auto color = window.at<cv::Vec3b>(vec[i].y, vec[i].x)[1];
            window.at<cv::Vec3b>(vec[i].y, vec[i].x)[1] = std::min(255.0f, color + 255.0f * ds[i] / sum_d);
        }
    }
}
